...
文件不存在时不会创建任何模型记录，也不会清理既有记录；内容为 `builtin_models: []` 时不会创建新记录，但会清理已经由 YAML 托管的旧记录。默认镜像不应携带部署专用模型。

每次应用启动会重新读取 YAML，按 `id` 字段 UPSERT 到 `models` 表（保留 `created_at`，刷新其他字段），并给这些行标记 `managed_by = 'yaml'`。随后 loader 会软删除 `managed_by = 'yaml'` 但已经不在当前 YAML 中的旧模型行。解析失败仅记录 Warning、不影响主流程，也不会执行清理。如果同 ID 模型已经由系统管理员在界面保存并转为运行时托管，则保留运行时配置，不再由 YAML 覆盖。

Web UI、API 或手工 SQL 创建的模型行应保持 `managed_by = ''`，不会被 YAML loader 触碰。如果把某个部署从 YAML 托管模型切换到 Web UI/SQL 托管模型，先把需要保留的模型行重新创建或把 `managed_by` 改为空，再把 YAML 改成 `builtin_models: []`；否则知识库、智能体或 GraphRAG 仍引用旧模型 id 时，会出现 `model not found`，前端可能表现为“实体关系提取失败”。
...
本地 Ollama 模型使用 `source: local`，`base_url` 与 `api_key` 留空，由 `OLLAMA_BASE_URL` 指向 Ollama 原生 API：
...
远程 OpenAI 兼容接口使用 `source: remote`，`base_url` 通常以 `/v1` 结尾：
...
```sql
SELECT id, name, type, is_builtin, managed_by, deleted_at, status
FROM models
...
9. **GraphRAG 模型引用**：实体关系抽取会读取已配置的聊天模型。若模型行被 YAML 清理或软删除，GraphRAG 会报 `model not found`，前端提示“实体关系提取失败”。先检查模型行的 `deleted_at` 和 `managed_by`，再决定是恢复模型行、重新在 UI 添加模型，还是把该模型重新写回 YAML。
