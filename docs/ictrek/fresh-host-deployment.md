...
2. 选定持久化方式，并把 `COMPOSE_FILE` 固定在 `.env` 中。
3. 准备 WeKnora 镜像，写好 `docker-compose.images.yml`。
...
先固定 compose 文件集合。新机器如果使用本目录里的 `data/*` 持久化，推荐：

```env
COMPOSE_FILE=docker-compose.yml:docker-compose.override.yml:docker-compose.images.yml
```

如果你接手的是一套已经用 named volume 跑起来的旧部署，不要改成 override 目录挂载，固定为：

```env
COMPOSE_FILE=docker-compose.yml:docker-compose.images.yml
```

选定后不要混用。后续所有 `docker compose` 命令都让 Compose 从 `.env` 读取 `COMPOSE_FILE`，或手工传入完全相同的 `-f` 集合。
...
上面的命令依赖 `.env` 中的 `COMPOSE_FILE`。如果没有在 `.env` 中固定，就必须每次手工传入同一组 `-f`。
...
2. Choose the persistence mode and pin `COMPOSE_FILE` in `.env`.
3. Prepare WeKnora images and write `docker-compose.images.yml`.
...
Pin the compose file set first. For a fresh host using the `data/*` directories
from this deployment directory, use:

```env
COMPOSE_FILE=docker-compose.yml:docker-compose.override.yml:docker-compose.images.yml
```

If you inherited an existing deployment that has always used named volumes, do
not switch it to override directory mounts. Keep:

```env
COMPOSE_FILE=docker-compose.yml:docker-compose.images.yml
```

After choosing the file set, do not mix modes. Let Docker Compose read
`COMPOSE_FILE` from `.env`, or pass the exact same `-f` set every time.
...
This assumes `COMPOSE_FILE` is pinned in `.env`. If it is not, pass the same
explicit `-f` set every time.
