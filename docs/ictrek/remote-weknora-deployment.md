...
最终答案合成由 `WEKNORA_CHAT_MODEL_CONTEXT_TOKENS`、`WEKNORA_CHAT_CONTEXT_SAFETY_TOKENS`、`WEKNORA_AGENT_FINAL_ANSWER_MAX_TOKENS` 共同控制；输出 token 过大时会按上下文窗口自动夹紧。`WEKNORA_CONVERSATION_MAX_COMPLETION_TOKENS` 控制普通知识库问答摘要/回答生成的输出 token。
