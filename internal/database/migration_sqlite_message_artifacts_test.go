...
// still keeps artifacts on messages.artifacts, checks 000029 copies them into
...
func TestSQLiteMessageArtifactsBackfillAndRollback(t *testing.T) {
	...
	down, err := os.ReadFile(filepath.Join(repoRoot, "migrations", "sqlite", "000029_message_artifacts_table.down.sql"))
	require.NoError(t, err)
