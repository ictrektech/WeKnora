...
func TestValidateTenantAPIKeyRequestRequiresCapabilitiesForScopedKey(t *testing.T) {
	h := &TenantHandler{}
	err := h.validateTenantAPIKeyRequest(context.Background(), 1, tenantAPIKeyCreateRequest{
		Name:       "integration",
		FullAccess: false,
	}, false)
	if err == nil {
...
func TestValidateTenantAPIKeyRequestAllowsFullAccessWithoutCapabilities(t *testing.T) {
	h := &TenantHandler{}
	if err := h.validateTenantAPIKeyRequest(context.Background(), 1, tenantAPIKeyCreateRequest{
		Name:       "owner",
		FullAccess: true,
	}, false); err != nil {
...
func TestValidateTenantAPIKeyRequestPersonalKeyRequiresKnowledgeBaseScope(t *testing.T) {
	h := &TenantHandler{}
	if err := h.validateTenantAPIKeyRequest(context.Background(), 1, tenantAPIKeyCreateRequest{
		Name:         "personal chat",
		Capabilities: []string{"chat"},
	}, true); err == nil {
		t.Fatal("expected personal API key without knowledge base scope to be rejected")
	}
}
