...
// reparseKnowledge performs the reparse workflow. Callers that have already
// dequeued stale tasks before changing the source row can skip that step so a
// replacement does not submit the same cancellation twice.
func (s *knowledgeService) reparseKnowledge(
	ctx context.Context,
	knowledgeID string,
	processOverrides *types.KnowledgeProcessOverrides,
	tasksAlreadyDequeued bool,
) (*types.Knowledge, error) {
