...
func (h *HousekeepingService) runSweep(ctx context.Context) {
	...
	if err := h.db.WithContext(ctx).
		...
		drainedFinalizing, skipped, _ := h.filterOutQueued(ctx, drainedFinalizing)
		if skipped > 0 {
			logger.Infof(ctx,
	...
	if err := h.db.WithContext(ctx).
		...
		staleCounterFinalizing, skipped, _ := h.filterOutQueued(ctx, staleCounterFinalizing)
		if skipped > 0 {
			logger.Infof(ctx,
	...
	if err := h.db.WithContext(ctx).
		...
		drainedSummary, skipped, _ := h.filterOutQueued(ctx, drainedSummary)
		if skipped > 0 {
			logger.Infof(ctx,
