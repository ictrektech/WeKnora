...
func RegisterTenantRoutes(
	...
	tenantRoutes := r.Group("/tenants")
	{
		...
		tenantByID := tenantRoutes.Group("/:id", g.PathTenantMatch())
		{
			...
			tenantByID.GET("/api-keys", g.Viewer(), handler.ListAPIKeys)
			tenantByID.POST("/api-keys", g.Viewer(), handler.CreateAPIKey)
			tenantByID.PUT("/api-keys/:key_id", g.Viewer(), handler.UpdateAPIKey)
			tenantByID.DELETE("/api-keys/:key_id", g.Viewer(), handler.DeleteAPIKey)
