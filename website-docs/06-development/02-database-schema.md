...
```text
migrations/
├── versioned/     # PostgreSQL/ParadeDB 版本化迁移：000000-000132
├── sqlite/        # SQLite 迁移：000000_init（压平的全量 schema）+ 000001-000040 增量版本
├── paradedb/      # ParadeDB 附加脚本：00-init-db.sql、01-migrate-to-paradedb.sql（存量库切换）
└── mysql/         # 00-init-db.sql，遗留的一次性 MySQL 建表脚本（未接入代码）
```

- PostgreSQL 的 `versioned/` 从 `000000_init` 到 fork 迁移 `000132_im_channel_locale`；上游新增迁移在 fork 中追加为 `000131` 和 `000132`。
- `sqlite/` 以 `000000_init` 作为压平后的全量初始化（JSONB→TEXT、SERIAL→AUTOINCREMENT 等方言差异已适配），增量迁移到 fork 版本 `000040_im_channel_locale`，同样由 golang-migrate 顺序执行。
- `uuid-ossp`、`vector`、`pg_trgm`、`pg_search` 等扩展由 `versioned/` 迁移自行创建；BM25 索引使用中文 Lindera 分词器建在 `embeddings.content` 上。
- `paradedb/00-init-db.sql` 含旧版建表语句，**没有被 compose 或代码引用**。不要把它挂成数据库 initdb 脚本：它先建出的旧表会让 `000000_init` 失败。
...
| 000068-000071 | 存储/资源/临时文档与平台级 API key | `storage_backends`、`resources`、`resource_bindings`、`resource_access_grants`、`temporary_documents`、平台级 API key |
| 000072-000074 | 内置 Agent prompt 与模型/主体修复 | prompt 更新、模型 Hub 别名修复、principal model 修复 |
...
| 000079 | 上游合并修复 | Wiki 与租户 API key 兼容修复 |

### Fork 迁移编号（000080–000132） {#_2-2-fork-迁移编号-000080–000132}

迁移编号以下表中的 fork 版本为准。上游最近增加的 PG 迁移经重编号后与 fork 的对应关系列在“上游 PG”列。

| Fork PG | 上游 PG | 变更 |
| --- | --- | --- |
| 000080 | — | `knowledges` 外部 ID 元数据索引；修复授权时间戳时区 |
| 000081 | — | 删除 `wiki_log_entries`，并为审计日志增加知识库活动范围 |
| 000082 | — | 分块编辑字段与历史表；MCP OAuth 刷新租期 |
| 000083–000085 | — | 用户模型脱敏、个人模型及脱敏服务 URL |
| 000086–000087 | — | 知识库目录路径与自动标签配置 |
| 000088–000090 | — | 技能快照计划名称、租户沙箱配置及会话沙箱配置 |
| 000091–000093 | — | 长期记忆、图像脱敏、消息用量 |
| 000094–000099 | — | 技能安装与快照账本、安装会话定位、API key 用户归属、消息附件、技能环境变量与目录 |
| 000100 | 000091 | `mcp_tool_approvals.enabled`，默认启用 |
| 000101–000104 | — | 合同审查表、模型选择、质量与证据字段及修复 |
| 000105 | — | 技能快照 `planned_name` 补列，兼容表创建顺序和已发布迁移 |
| 000106–000107 | — | 法律工作区配置与幂等修复 |
| 000108 | 000092 | `mcp_metadata` 表与 `mcp_services.usage_instructions` |
| 000109–000110 | — | Smart Archive 核心表、会话 `workspace_mode` |
| 000111 | 000093 | 浏览器设备授权与配对 |
| 000112–000113 | 000094–000095 | 记忆一致性修复、数据库向量检索 |
| 000114 | 000096 | 钉钉渠道切换为 websocket（Stream） |
| 000115–000116 | 000097–000098 | 会话分叉与沙箱快照租约 |
| 000117 | 000099 | ParadeDB `pg_search` 升级到 0.22.6；中断重跑前须删除无效索引 |
| 000118 | 000100 | 删除拖慢写入的三个 `chunks` 索引 |
| 000119 | 000101 | 文档画像与知识库生成描述 |
| 000120 | 000102 | `mcp_endpoints`，空间对外发布的 MCP 端点 |
| 000121 | 000103 | `message_artifacts` 表；从 `messages.artifacts` 回填后切换读写，保留旧列供回滚和滚动升级 |
| 000122 | 000104 | `tenant_skills.served`，保留升级期间仍在镜像中的技能版本 |
| 000123–000124 | 000105–000106 | 消息上下文 checkpoint 与会话历史索引 |
| 000125 | 000107 | `message_artifacts.deleted_at` 墓碑列 |
| 000126–000129 | — | Smart Archive 镜像状态、取消状态、租约与抽取进度 |
| 000130 | 000108 | `sessions.sandbox_config_tenant_id`，记录沙箱配置所属空间 |
| 000131 | 000109 | `sessions.host_workspace_dir`，Lite 桌面版主机沙箱项目目录 |
| 000132 | 000110 | `im_channels.locale`，固定渠道回复语言；空串使用部署默认值 |
...
| 000012–000013 | 消息 usage（PG 000093）、MCP 工具 enabled（PG 000100） |
| 000014–000016 | 浏览器授权（PG 000111）、记忆一致性（PG 000112）、记忆向量检索（PG 000113；SQLite 无向量列，仍在应用内排序） |
| 000017 | 钉钉渠道改为 websocket（PG 000114） |
| 000018–000019 | 法律工作区配置（PG 000106–000107）、合同审查表（PG 000101–000104） |
| 000020–000021 | 合同审查模型与质量字段（PG 000102–000104） |
| 000022–000023 | Smart Archive 基础表（PG 000109）、会话 `workspace_mode`（PG 000110） |
| 000024–000025 | 会话分叉（PG 000115）、分叉快照租约（PG 000116） |
| 000026–000027 | `chunks` 索引精简（PG 000118）、知识画像（PG 000119） |
| 000028 | `mcp_endpoints`（PG 000120） |
| 000029 | `message_artifacts` 表，回填并清空旧列（PG 000121） |
| 000030–000031 | `messages.context_checkpoint`（PG 000123）、会话历史索引（PG 000124） |
| 000032 | `message_artifacts.deleted_at` 墓碑列（PG 000125） |
| 000033–000036 | Smart Archive 镜像状态、取消状态、租约与抽取进度（PG 000126–000129） |
| 000037 | `sessions.sandbox_config_tenant_id`（PG 000130；Lite 无共享空间，不回填） |
| 000038 | 补建 `tenant_skills`、`tenant_skill_snapshots`、`tenant_skill_catalog`、`tenant_user_env_vars`（fork PG 000088、000094、000095、000098、000099、000105、000122）。此前 Lite 缺这四张表，技能读取凭据时报 `no such table` |
| 000039–000040 | `sessions.host_workspace_dir`（PG 000131）、`im_channels.locale`（PG 000132） |

PG `000108`（`mcp_metadata`、`mcp_services.usage_instructions`）与 `000117`（pg_search 升级）没有 SQLite 对应版本。
...
| `auth_tokens` | 登录令牌 | `id`、`user_id`（FK→users，CASCADE）、`token`、`token_type`（access/refresh）、`expires_at`（TIMESTAMPTZ，000080 起）、`is_revoked` |
...
| `knowledges` | 知识条目（文档/网页/FAQ 等） | `id`、`tenant_id`、`knowledge_base_id`（FK）、`type`、`title`、`source`（VARCHAR(2048)）、`parse_status`（pending/processing/finalizing/completed/failed/cancelled/deleting）、`pending_subtasks_count`（000056，`finalizing` 阶段未完成的增强子任务数）、`enable_status`、`file_name`/`file_type`/`file_size`/`file_path`/`file_hash`、`metadata`（内部入库状态）、`custom_metadata`（JSONB，用户自填元数据，000078）、`folder_path`（目录树路径，000086）、`summary_status`、`profile`（JSONB，文档画像：gist/主题/类型/典型问题，000119）、`channel`、`processed_at`/`error_message`。**没有 `tag_id` 列**——000063 起标签走 `knowledge_tag_relations` 关联表 |
...
| 表 | 用途 | 关键字段 |
| --- | --- | --- |
| `sessions` | 会话（对话上下文与检索参数快照） | `id`、`tenant_id`、`title`、`knowledge_base_id`、`agent_id`（FK→custom_agents）、`user_id`、`max_rounds`、`enable_rewrite`、`fallback_strategy`/`fallback_response`、`keyword_threshold`/`vector_threshold`、`embedding_top_k`/`rerank_top_k`/`rerank_threshold`、`rerank_model_id`/`summary_model_id`、`agent_config`/`context_config`（JSONB）、`sandbox_config_id`/`sandbox_config_tenant_id`（0 表示会话自身空间，000130）、`workspace_mode`（会话所属产品区，000110）、`parent_session_id`/`forked_from_message_id`/`fork_bootstrap`（会话分叉，000115；不设外键）、`host_workspace_dir`（Lite 桌面版项目目录，000131） |
| `messages` | 消息 | `id`、`request_id`、`session_id`（FK）、`role`、`content`/`rendered_content`、`knowledge_references`（JSONB 引用）、`agent_steps`（JSONB，Agent 推理轨迹）、`mentioned_items`/`images`（JSONB）、`is_completed`/`is_fallback`、`channel`（web/IM 渠道）、`agent_id`+`agent_tenant_id`、`model_id`、`knowledge_id`、`agent_duration_ms`、`execution_context`、`attachments`、`used_memories`/`usage`（JSONB）、`sandbox_checkpoint`（分叉点的沙箱检查点，000115）、`context_checkpoint`（Agent 上下文压缩摘要，000123）。`artifacts` 列自 000121 起不再读写且已清空（仅为回滚与滚动升级保留），生成文件见 `message_artifacts` |
| `message_artifacts` | 技能生成的文件，一行一个（000121） | `session_id`、`message_id`、`position`（消息内序号，即下载接口的 index；与 `message_id` 唯一）、`url`（存储或 resource:// 引用，不返回客户端）、`file_name`/`file_type`/`file_size`、`content_hash`、`source_path`（同会话同路径视为同一文件的多个版本）、`mod_time`（RFC 3339 文本，保留纳秒精度供采集器比对）、`created_at`、`deleted_at`（000125，删除后保留墓碑行，保持 `position` 稳定且不被重新采集）。消息被软删除时其文件一并隐藏 |
...
| `mcp_services` | MCP 服务配置 | `id`、`tenant_id`、`name`、`enabled`、`transport_type`（stdio/sse/…）、`url`/`headers`/`auth_config`/`advanced_config`/`stdio_config`/`env_vars`（JSONB）、`is_builtin`、`usage_instructions`（本地维护的使用说明，000108） |
| `mcp_metadata` | MCP 工具目录快照（000108） | PK（`tenant_id`,`service_id`,`principal`），静态认证时 `principal` 为空串、OAuth 服务按用户主体分行；`config_fingerprint`、`tools`（JSONB）、`instructions`、`server_name`/`server_version`/`server_description`、`synced_at`；随服务删除级联 |
| `mcp_endpoints` | 空间对外发布的 MCP 端点（000120） | `tenant_id`、`name`/`description`、`enabled`、`token_hash`（Bearer token 的 SHA-256，未删除行唯一，明文只显示一次）/`token_hint`、`knowledge_base_ids`（JSONB，空数组表示全部知识库）、`tools`（JSONB 白名单）、`default_agent_id`、`rate_limit_per_minute`（默认 60）、`last_used_at` |
| `mcp_tool_approvals` | MCP 工具审批策略（000042） | (`tenant_id`,`service_id`,`tool_name`) 唯一、`require_approval`、`enabled`（默认 true，000100） |
| `mcp_oauth_clients` | MCP OAuth 客户端（000062） | (`tenant_id`,`service_id`) 唯一、`client_id`/`client_secret`/`redirect_uri` |
| `mcp_oauth_tokens` | MCP OAuth 令牌 | (`tenant_id`,`principal_type`,`principal_id`,`service_id`) 唯一（000064 起按主体，原按 `user_id`）、`access_token`/`refresh_token`、`expires_at`、`refresh_lease_id`/`refresh_lease_until`（000082，防并发刷新） |

### 浏览器连接

| 表 | 用途与关键字段 |
| --- | --- |
| `browser_devices` | 已授权的浏览器扩展设备（000111），每个空间内每个用户一台；label、token_hash/previous_hash（只存 SHA-256，令牌轮换有宽限期）、expires_at/renew_after、last_seen_at、revoked_at、在线租约 owner/lease_until |
...
| `tenant_skills` | catalog_id 与 sandbox_config_id 对应一次安装；enabled/status/error、installed_snapshot_id、installing_since、install_session_id/install_message_id、envs、served（新版本安装中或失败时仍在提供的版本，000122） |
...
| `fork_snapshot_leases` | snapshot_id（PK）、tenant_id、sandbox_config_id、created_at；会话分叉时先记下沙箱快照，分叉失败或进程中断后由后台回收（000116） |
...
| 表 | 用途与关键字段 |
| --- | --- |
| `memory_subjects` | (tenant_id,subject_id) 唯一；个人 enabled、常驻 block_text、item_count、extract_cursor/pending_sessions/extract_scheduled_at、extraction_state（000112）、整理时间 |
| `memory_extraction_sessions` | PK（tenant_id,subject_id,session_id）；按会话记录抽取游标、pending 标记与失败区间（000112） |
| `memory_items` | kind/content/topic/normalized_key、importance/origin/status、来源会话/消息、valid_from/invalid_at/expires_at、superseded_by、replaces_id（待确认条目要替换的旧条目，000112） |
...
| `memory_item_embeddings` | item_id、model_id、dims、vector；PostgreSQL 装有 `vector` 扩展时另有 embedding（halfvec，000113）供库内检索，SQLite 在应用内排序；与条目分表存储 |
...
| `im_channels` | IM 渠道接入配置（企业微信/飞书/Slack 等） | `tenant_id`、`platform`、`agent_id`、`knowledge_base_id`、`mode`（websocket/webhook；钉钉自 000114 起只用 websocket）、`output_mode`、`session_mode`、`bot_identity`、`credentials`、`locale`（固定回复语言，空串用部署默认值，000132） |
...
1. **创建文件**：`make migrate-create name=add_my_feature`，在 `migrations/versioned/` 下生成下一个版本号（当前最大为 `000132`；创建前再次检查目录，使用下一个空闲版本的 up/down 文件）；
...
官方镜像自 v0.8.2 起为 `paradedb/paradedb:v0.22.6-pg17`。迁移 000117 只把已安装的 pg_search 0.22.2–0.22.5 升级到 0.22.6：若启动日志提示 `pg_search 0.22.6 is not available`，说明数据库镜像还没换，换镜像后手工执行 `ALTER EXTENSION pg_search UPDATE TO '0.22.6'`；其他版本线不会被自动升级或降级。
