...
fork 迁移 `000117` 仅在 WeKnora 数据库内执行扩展升级：已安装版本须为 0.22.2–0.22.5，且服务器提供 0.22.6 扩展包。它遵守 `app.skip_embedding`，不安装缺失的扩展，也不处理其他版本线。

如果这条迁移在替换数据库镜像**之前**已经执行，安装新镜像后不会自动再跑一次，需要手动执行上面的 SQL。没有 `000117` 的旧应用也需手工升级。其他数据库的扩展不能靠 WeKnora 的迁移代管。

## 回滚与复现验证

回滚需要将升级前备份/快照恢复到独立数据卷，配合旧镜像使用。仅把镜像标签改回旧版不会撤销扩展 SQL 变更，`000117` 的 down 文件也不会尝试降级扩展。
