...
ictrek 镜像默认提供 `builtin_models: []`，不写入某台部署机器的模型后端。这个空列表会软删除此前仍标记为 `managed_by='yaml'` 的模型；已有部署改用空列表前，应先在界面重建仍被知识库、智能体或 GraphRAG 引用的模型，或把要保留的行转为运行时托管（`managed_by=''`）。否则引用会报 `model not found`。文件不存在时不执行这次清理；解析失败也只记录警告并保留旧行。手工 SQL 插入的模型不应标记为 `managed_by='yaml'`。
