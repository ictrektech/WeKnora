-- Migration 000113: move memory vector search into the database.
--
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE '[Migration 000113] memory_item_embeddings absent · skipping';
        RETURN;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
        RAISE NOTICE '[Migration 000113] vector extension absent · memory keeps scoring in process';
    ELSE
        ALTER TABLE memory_item_embeddings ADD COLUMN IF NOT EXISTS embedding halfvec;
        RAISE NOTICE '[Migration 000113] memory_item_embeddings.embedding ready';
    END IF;
