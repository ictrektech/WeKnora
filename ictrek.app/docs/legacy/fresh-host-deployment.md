...
`/data/jhu` 不是强制路径，可以换成本机实际数据盘。不要从源码根目录复制默认 `docker-compose.yml`；部署目录应使用 `ictrek.app/docs/legacy/deploy-template/docker-compose.yml`。
...
从 `ictrek.app/docs/legacy/deploy-template/.env.example` 开始：
...
然后在 `ictrek.app/docs/legacy/deploy-template/docker-compose.yml` 复制出来的部署文件中取消挂载注释：
...
生产或远程空机器上，优先使用 `ictrek.app/docs/legacy/deploy-template/docker-compose.yml`：
...
`ictrek.app/docs/legacy/deploy-template/docker-compose.yml`.
...
mount in the copied `ictrek.app/docs/legacy/deploy-template/docker-compose.yml`:
...
Use `ictrek.app/docs/legacy/deploy-template/docker-compose.yml` for a normal released-image
deployment:
