...
本文面向工作区管理员和部署维护者，说明法律工作台的入口开关、数据保留和数据删除边界。合同审查的功能、状态和结果判读见 [合同审查说明](../../docs/contract-review.md)；智能档案的导入、队列和提醒见 [智能档案说明](smart-archive.md)。
...
| 页面级 drill-down 子导航“合同审查 / 智能档案” | 已实现；进入 `/platform/contract-review` 或 `/platform/smart-archive` 后替换平台侧栏内容 |
| 设置入口 | 已实现；设置 → 发布集成 → 法律工作台 |
| 关闭后的数据行为 | 已实现；只隐藏入口并阻止合同审查、智能档案访问，不删除数据；提醒调度跳过已关闭租户 |
...
4. 开启后，左侧出现“法律工作台”；点击后进入 `/platform/contract-review`，侧栏只显示返回平台导航、法律工作台、合同审查和智能档案。
5. 关闭后，左侧入口隐藏；正在访问的工作台会返回知识库列表，服务端后续合同审查和智能档案请求返回 `403`。重新开启后，保留的数据可以继续访问，提醒调度会恢复有效事件。
