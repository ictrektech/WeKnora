...
本文面向普通使用者，说明如何创建知识库、配置模型和解析能力、上传文档、重新解析、下载文档，以及在对话中使用知识库引用。部署、并发和模型资源配置属于管理员工作，见 [VOS 应用说明](../README.md) 和 [Model Hub 预热说明](vos-ollama-prewarm.md)。旧独立部署模板只保存在 [legacy](legacy/) 中供备查，不再作为当前部署入口。
...
- 通过部署配置、环境变量或初始化接口预置且没有用户归属的模型是共享模型，空间内用户均可使用；普通用户不能修改或删除。
- 任何用户通过界面或登录用户 API 新增的模型都是该创建者的个人模型。管理员通过界面新增的模型也属于管理员本人，不会自动成为共享模型。
- 个人模型只对创建者可见、可用、可编辑和可删除；其他用户即使添加相同名称、Base URL 和底层模型，也会得到彼此独立的模型记录。
- 升级前已经存在且没有归属信息的模型继续按共享模型处理，避免升级后现有知识库失去模型。
- 如需给所有用户增加共享模型，应使用部署预配置方式，不要使用个人账号在界面中新增。

### 按用户、按模型启用请求脱敏

在「模型配置」中找到对话或视觉模型，点击模型卡片上的「个人脱敏设置」；具有模型编辑权限的用户也可以在编辑模型的「高级选项」中设置。该设置按“当前用户 + 当前模型记录 ID”独立保存，默认关闭：

- 同一用户的 QA 与 VLM 设置互不影响；即使两个模型使用相同的名称、服务地址或底层 Ollama 模型，也不会联动。
- 同一个模型的不同用户各自保存设置，一个用户的修改不会改变其他用户。
- 脱敏服务地址也按“当前用户 + 当前模型记录 ID”保存。普通用户可在「个人脱敏设置」中填写，并可一键复制同一 `vos_default` 网络中的内置服务地址。
- 规则脱敏会在请求发往该模型前替换手机号、邮箱、身份证、密钥等结构化敏感信息。
- NER 是规则脱敏后的增强层级，用于补充识别人名和地点；关闭规则脱敏时 NER 也会关闭。
- 图片脱敏是另一个增强层级：聊天界面上传的图片和送入该模型解析的图片会先通过脱敏服务的 OCR 识别并遮挡敏感区域，再把脱敏后的图片发给模型；关闭规则脱敏时图片脱敏也会关闭。远程 `http(s)` 图片链接不做下载脱敏，会原样传递。
- 普通聊天、历史对话中的文字、多模态聊天文字和图片解析提示词均执行当前用户对相应模型设置的策略。

如果 HybRAG 与脱敏应用均安装在同一个 VOS 的 `vos_default` 网络中，脱敏服务地址填写：

```text
http://desensitize-backend:5000
```

VOS Compose 已默认将 `desensitize-backend` 加入 SSRF 白名单。自定义覆盖 `SSRF_WHITELIST_EXTRA` 时也必须保留该名称。启用脱敏后若服务不可用，模型请求会被阻止，不会退回为直接发送原文。
...
操作方法：普通用户一般不需要手动修改这里。只有管理员要求下载备用模型时，才在「下载新模型」里输入模型名称并点击下载。模型是否常驻、是否使用 GPU，由管理员在 Model Hub 中配置；HybRAG 默认只引用 Model Hub 已预热的 QA/VLM 和 embedding Gateway。
...
回答生成期间可以切换到其他对话，再返回当前对话继续查看。系统会保留正在生成的回答和阅读位置，并把已生成内容增量保存到会话记录，避免历史刷新或页面切换把较长回答截断。流式阶段按纯文本逐字显示，回答完成后再渲染 Markdown 和引用。

回答完成后，系统会异步生成会话标题，并优先使用本轮回答实际使用的模型。标题模型暂时不可用时，会把首条问题压缩为兜底标题；这不会影响回答保存，也不会让已完成会话长期显示为「新会话」。
...
并发、模型显存、后台任务优先级由管理员配置。机器资源不足时，不建议普通用户同时开启多模态、Wiki 和图谱解析大量文档；当前 VOS app 默认资源说明见 [VOS 应用说明](../README.md#资源默认值)。

## 15. 系统资源与任务队列

此部分仅供管理员使用。进入「系统设置」可查看后台任务池配置；修改带「需重启」标识的 worker 参数后，必须重启 app 才会重建任务池。VOS app 的默认资源、模型服务和安装依赖说明见 [VOS 应用说明](../README.md)。

![系统资源设置](user_guide_img/longshot20260714095505.png)

六个 worker 池分别负责：核心解析（文档解析、分块、向量化）、后处理编排、内容富化（摘要、多模态、图谱、问题生成）、维护与同步、共享弹性，以及 Wiki。它们控制可同时领取的后台任务数，不等于模型并发。core、postprocess、enrichment、maintenance 和 wiki 池要求正整数；设置为 `0` 或负数会回退到代码默认值。shared 是可选弹性池，可以设为 `0` 关闭借用。小机器从专用池 `1`、shared `1` 开始，thor 这类大机器可按部署模板把 shared 设为 `0`。
