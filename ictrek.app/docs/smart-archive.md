...
| 外部通知、ERP/自定义字段同步、LexAI 独立 `/legal` 壳 | 未实现；不属于当前 WeKnora 集成范围 |
...
2. 从侧栏进入“法律工作台 → 智能档案”。
3. 选择“批量导入”，上传受支持的文件。上传接口立即返回批次；页面通过 SSE 和轮询显示进度。
...
智能档案托管知识库由系统维护，描述带有 `[weknora-managed-smart-archive]` 标记。普通知识库接口拒绝直接编辑或删除该知识库；导入、重试和清理通过智能档案服务的内部写路径执行。源文件通过资源目录绑定，只有最后一个引用释放后才删除。
...
- SQLite/Lite 使用 `migrations/sqlite/000020_smart_archive.up.sql`。
- 解析共享结果存储在 `document_parse_artifacts`；回滚迁移会按依赖顺序删除智能档案表。
- 已验证：`go test ./internal/types ./internal/application/repository -count=1`、后端路由/服务/容器编译、SQLite migration 聚焦测试，以及前端 `npm run type-check`、`npm run check-i18n`、智能档案状态测试和生产构建。
