...
安装时选择一个 profile：`amd`、`arm`、`l4t` 或 `thor-spark`。

AMD 和 ARM 通用 profile 分别从 `AMD_with_cuda`、`ARM_with_cuda` 飞书表读取 `weknora*` 与 `ollama_server` 镜像版本；`l4t`、`thor-spark` 使用各自表格。本应用只发布 4 个 profile。
...
启动时，两个 Ollama 容器会先启动本地 `ollama serve`，再通过 `MODEL_HUB_BACKEND_URL` 触发 Model Hub 拉取模型；如果 Model Hub 不可用或任务失败，则退回本容器 `ollama pull`。拉取完成后会用 `OLLAMA_KEEP_ALIVE=-1` 发起 warmup，让模型常驻，然后才启动 OpenAI-compatible gateway。默认 `MODEL_HUB_BACKEND_URL=http://model-hub-backend:5005`，这是 `vos_default` 网络内的 Model Hub 后端 alias。
