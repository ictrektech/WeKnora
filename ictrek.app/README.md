...
这等价于 Ollama 容器内 `/root/.ollama/models`。挂载保持可写，因为 WeKnora 也有通过 Ollama 拉取模型的能力；这些模型会写回 Model Hub 的共享目录。WeKnora 不再用自己的 `${VOS_APP_STORAGE_PATH}/ollama/qa` 或 `${VOS_APP_STORAGE_PATH}/ollama/embedding` 存模型，避免与 Model Hub 下载目录分裂。安装时如 Model Hub 使用了非默认共享目录，需要把 WeKnora 的 `MODEL_HUB_SHARED_MODELS_PATH` 配成同一个宿主机路径。
