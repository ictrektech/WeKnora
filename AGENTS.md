...
- Keep Feishu credentials separated by operation: `build_image.sh` writes release tags and must use the write-capable `~/.feishu.json`; `~/.feishu.components.json` is read-only and is only for VOS packaging/version lookup. Never override a build with `FEISHU_CONFIG_FILE=~/.feishu.components.json`.
