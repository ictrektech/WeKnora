...
import { ref } from 'vue'
import { archiveDocument, bulkArchiveDocuments, bulkDeleteArchiveDocuments, bulkDeleteArchiveReminders, bulkIgnoreArchiveReminderCandidates, bulkPurgeArchiveDocuments, bulkRestoreDocuments, cancelArchiveDocumentExtraction, createArchiveReminderFromCandidate, deleteArchiveDocument, deleteArchiveNotification, getArchiveBatch, getArchiveDocument, getArchiveSettings, importArchiveFiles, listArchiveCustomers, listArchiveDocuments, listArchiveReminderCandidates, listArchiveReminders, listArchiveNotifications, markArchiveNotificationRead, restoreDocument, retryArchiveDocumentExtraction, retryArchiveDocumentMirror, searchArchive, streamArchiveBatch, updateArchiveDocument, updateArchiveReminder, type ArchiveBatch, type ArchiveBulkActionResult, type ArchiveCustomer, type ArchiveDocument, type ArchiveNotification, type ArchiveReminder, type ArchiveReminderCandidate, type ArchiveSearchFilters, type ArchiveSearchResponse, type ArchiveSettings } from '@/api/smart-archive'
...
export const useSmartArchiveStore = defineStore('smartArchive', () => {
  ...
  async function cancelExtraction(id: string) { const row = (await cancelArchiveDocumentExtraction(id)).data; documents.value = documents.value.map((item) => item.id === id ? row : item); if (current.value?.id === id) current.value = row; return row }
  async function retryMirror(id: string) { const row = (await retryArchiveDocumentMirror(id)).data; documents.value = documents.value.map((item) => item.id === id ? row : item); if (current.value?.id === id) current.value = row; return row }
  ...
  return { documents, customers, reminders, reminderCandidates, notifications, settings, current, searchResult, searchPage, searchTotal, loading, importProgress, loadDocuments, refreshDocuments, loadSettings, loadDocument, upload, loadEntities, loadReminders, loadReminderCandidates, createReminderFromCandidate, loadNotifications, markNotificationRead, deleteNotification, search, refreshSearch, updateDocument, retryExtraction, cancelExtraction, retryMirror, archive, restore, deleteDocument, bulkAction, updateReminder, bulkDeleteReminders, bulkIgnoreReminderCandidates, connectBatch, disconnect }
})
