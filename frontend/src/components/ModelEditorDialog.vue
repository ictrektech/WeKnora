...
const thinkingControlOptions = computed(() => {
  const keys = ['none', 'chatTemplateKwargs', 'enableThinking', 'thinkingType', 'think', 'reasoningEffort'] as const
  const values = ['none', 'chat_template_kwargs', 'enable_thinking', 'thinking_type', 'think', 'reasoning_effort'] as const
...
const handleConfirm = async () => {
  ...
  try {
    if (personalOnly.value) {
      ...
      const personalPayload = { ...formData.value, personalOnly: true }
      if (props.saveModel) {
        await props.saveModel(personalPayload)
      } else {
        emit('confirm', personalPayload)
      }
    ...
    if (props.saveModel) {
      await props.saveModel(savePayload)
    } else {
      emit('confirm', savePayload)
    }
    emit('update:visible', false)
    invalidateConnectionTest(false)
