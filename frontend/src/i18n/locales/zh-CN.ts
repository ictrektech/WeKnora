export default {
  ...
  integrations: {
    ...
    api: {
      ...
      externalGuideDesc: '不在 VOS 里打开页面时，外部服务可以直接调用当前 HybRAG REST API。',
      externalGuideAuthTitle: '认证',
      externalGuideAuthDesc: '推荐为服务创建独立 API Key，并用 X-API-Key 调用；浏览器登录态也可以使用 Authorization: Bearer <token>。',
