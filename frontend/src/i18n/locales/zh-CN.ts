export default {
  ...
  integrations: {
    ...
    api: {
      ...
      externalGuideDesc: '不在 VOS 页面里打开时，外部服务也可以调用当前 HybRAG REST API。支持普通用户个人 API Key，也支持普通用户 VOS/OAuth Bearer token。',
      externalGuideAuthTitle: '认证',
      externalGuideAuthDesc: '方式一：普通用户在 API 集成里创建自己的个人 API Key，外部应用用 X-API-Key 调用。方式二：外部应用先从 VOS/OAuth 拿到该普通用户 token，再用 Authorization: Bearer <VOS token> 调用，HybRAG 会按该 VOS 用户身份和权限执行。',
      ...
      personalApiKeyAccessTypeHint: '普通用户创建的是个人 API Key，只对当前账号生效；可用于检索、问答和写入已选择的知识库，不能获得空间完全访问。',
