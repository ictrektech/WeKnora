export default {
  ...
  integrations: {
    ...
    api: {
      ...
      externalGuideAuthDesc: 'Option 1: an ordinary user creates a personal API key here, and the external app sends X-API-Key. Option 2: the external app obtains that user’s VOS/OAuth token and sends Authorization: Bearer <VOS token>; HybRAG verifies it and runs with the mapped user permissions.',
      ...
      personalApiKeyAccessTypeHint: 'Ordinary users create personal API keys. They only apply to the current account, can retrieve/chat/ingest selected knowledge bases, and cannot grant full workspace access.',
