export default {
  smartArchive: {
    ...
    bulkDeleteConfirm: '選択した {count} 件の文書を完全に削除しますか？関連するリマインダーも取り消されます。',
    bulkActionSuccess: '{count} 件を処理しました。',
    bulkActionPartial: '{succeeded} 件を処理しました。{failed} 件は失敗したため、一覧を更新して確認してください。',
    bulkActionFailed: '一括操作に失敗しました。',
    bulkDeleteReminders: 'リマインダーを一括削除',
    bulkDeleteRemindersConfirm: '選択した {count} 件のリマインダーを削除しますか？',
    bulkDeleteRemindersActiveConfirm: '選択した {count} 件のうち {active} 件が予約中です。削除すると今後の通知が停止します。続行しますか？',
    bulkDeleteRemindersSuccess: '{count} 件のリマインダーを削除しました。',
    bulkIgnoreCandidates: '選択項目を無視',
    bulkIgnoreCandidatesConfirm: '選択した {count} 件を無視しますか？リマインダーは作成されませんが、記録は保持されます。',
    ...
    invalidFiles: '対応していないファイルをスキップしました。PDF、Word、Excel、JPG、PNG、WEBPに対応しています。',
    loading: '読み込み中…',
    loadFailed: 'アーカイブを読み込めませんでした。',
    searchFailed: '検索に失敗しました。',
    documents: '文書',
    documentCount: '{count} 件の文書',
    documentsDescription: '契約書、借用契約、出庫記録と原文の根拠。画像OCRにも対応します。',
    customers: '顧客',
    customersDescription: '過去の文書から正規化された顧客情報。',
    reminders: 'リマインダー',
    remindersDescription: '内容を確認してからリマインダーを作成・有効化します。',
    candidatesTitle: '確認が必要な項目',
    candidatesDescription: '検出された期日は候補であり、自動的にはリマインダーを作成しません。',
    noCandidates: '確認が必要な項目はありません',
    candidateNeedsReview: '先にフィールドを確認してください',
    candidateReview: 'リマインダーを確認',
    createReminder: 'リマインダーを作成',
    reminderCreated: 'リマインダーを作成しました。準備ができたら有効化してください。',
    offsetDays: '何日前',
    reminderTime: '通知時刻',
    assignee: '担当者',
    ...
    emptyDocumentsDescription: 'PDF、Word、Excel、JPG、PNG、WEBPをインポートしてフィールドを抽出します。',
    noEntities: '記録はありません',
    ...
    noVerifiedFields: '確認可能なフィールドがありません。再認識するか手動で確認してください。',
    documentTypes: {
      contract: '契約書',
      loan_agreement: '借用契約',
      outbound_order: '出庫記録',
  ...
  integrations: {
    ...
    api: {
      ...
      directHostBaseUrlDesc: 'The VOS package maps the RAG API to host port 29081 by default, so LAN or external systems can call it directly. If the install port was changed, use the installed value.',
      externalGuideTitle: 'External API quick reference',
      externalGuideDesc: 'External services can call HybRAG through the VOS app path or through the direct host port. HybRAG supports both personal user API keys and VOS/OAuth bearer tokens.',
      externalGuideAuthTitle: 'Authentication',
      externalGuideAuthDesc: 'Option 1: an ordinary user creates a personal API key here, and the external app sends X-API-Key. A personal key only inherits that user’s knowledge-base scope and does not need an admin-issued key. Option 2: the external app obtains that user’s VOS/OAuth token and sends Authorization: Bearer <VOS token>; HybRAG verifies it and runs with the mapped user permissions.',
      externalGuideSessionTitle: 'Session',
      externalGuideSessionDesc: 'Create a session with POST /sessions first, then pass the returned id to /knowledge-chat/:session_id or /agent-chat/:session_id.',
      externalGuideKbTitle: 'Knowledge scope',
      externalGuideKbDesc: 'Pass knowledge_base_ids in the request body. API keys can also be restricted to selected knowledge bases when created.',
      externalGuideRagExample: 'RAG Chat example',
      externalGuideRagStep: '# 2. RAG chat (SSE; replace <session_id> with the id returned by step 1)',
      ...
      personalApiKeyAccessTypeHint: 'Ordinary users create personal API keys. They only apply to the current account, can retrieve/chat/ingest selected knowledge bases, cannot grant full workspace access, and do not need an admin-issued key.',
      ...
      personalApiKeyKnowledgeScopePlaceholder: 'Select the knowledge bases this personal API key may access',
      personalApiKeyKnowledgeScopeHint: 'Personal API keys must be bound to selected knowledge bases visible to the current user.',
      personalApiKeyKnowledgeScopeRequired: 'Select at least one knowledge base for a personal API key',
  ...
  datasource: {
    ...
    gitlab: {
      ...
      paths: 'ディレクトリ',
      pathsPlaceholder: '1行に1つのディレクトリを入力します。空欄の場合はプロジェクト全体を同期します',
  ...
  model: {
    ...
    editor: {
      ...
      desensitizeServiceUrlRequired: 'A redaction service URL is required when redaction is enabled',
  ...
  system: {
    ...
    vosAppVersionDescription: 'Version of the installed VOS application package',
    ...
    docreaderImageVersionDescription: 'Version of the running weknora-docreader image',
    sandboxImageVersionLabel: 'Sandbox Image Version',
    sandboxImageVersionDescription: 'Version of the configured weknora-sandbox image',
  ...
  settings: {
    ...
    sandbox: {
      ...
      disableScriptsConfirm: 'このワークスペースのすべてのエージェントが、サンドボックスでスキルスクリプトを実行しなくなります。スキルの内容を読むことは引き続き可能です。既存のリモートサンドボックスは自動的には破棄されません。関連するセッションを終了または削除して解放してください。続行しますか？',
  ...
  contractReview: {
    ...
    subtitle: 'Upload a contract to review risks, recommendations, and evidence by clause.',
    ...
    uploadFirstDescription: 'Upload a PDF or DOCX file before starting a review.',
    model: 'Review model',
    ...
    reconfigureDescription: 'Change the represented party, playbook, or model and run again.',
    cancel: 'Cancel',
    cancelReview: 'Cancel review',
    cancelReviewConfirm: 'Cancel this contract review? Partial results will be kept and you can retry it later.',
    ...
    evidenceLocatorUnsupported: 'Stable evidence locations are unavailable; reliable highlighting is disabled.',
    evidenceLocatorFailed: 'Evidence locations failed to load; stable highlights are temporarily unavailable.',
    ...
    summaryUnavailable: 'The overview is unavailable; verify the finding details.',
    qualityWarnings: 'Result verification notes',
    summaryDetailsMismatch: 'The overview risk counts do not match the finding details.',
    issuesNotLoaded: 'Finding details have not loaded, so no issues cannot be confirmed.',
    summaryMissing: 'The review is complete, but no overview summary was provided.',
