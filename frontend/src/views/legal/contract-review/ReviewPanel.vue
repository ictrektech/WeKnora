<template>
  <aside class="review-panel" :aria-label="t('contractReview.reviewPanelTitle')">
    ...
    <template v-else>
      ...
      <div v-if="review.status === 'failed'" class="review-error" role="alert"><strong>{{ t('contractReview.reviewFailed') }}</strong><p>{{ review.error_message || t('contractReview.errorDetailsUnavailable') }}</p><button v-if="review.error_message?.toLowerCase().includes('model')" type="button" @click="emit('configure')">{{ t('contractReview.configureModel') }}</button><button data-testid="contract-retry-review" type="button" @click="emit('retry')">{{ t('contractReview.retry') }}</button></div>
      ...
      <div class="review-panel__content">
        ...
        <section v-else-if="activeTab === 'issues'" class="issues-pane" role="tabpanel">
          ...
          <article v-for="issue in sortedIssues" :id="`review-issue-${issue.id}`" :data-testid="`contract-issue-${issue.id}`" :key="issue.id" class="issue" :class="{ selected: selectedIssueId === issue.id }" tabindex="0" role="button" :aria-label="issueAriaLabel(issue)" :aria-pressed="selectedIssueId === issue.id" @click="selectIssue(issue)" @keydown="onIssueKeydown($event, issue)">
            ...
            <div v-if="evidenceCandidateCount(issue) > 1" class="evidence-candidates" role="group" :aria-label="t('contractReview.evidenceCandidates', { count: evidenceCandidateCount(issue) })">
              <span>{{ t('contractReview.evidenceCandidates', { count: evidenceCandidateCount(issue) }) }}</span>
              <button v-for="candidate in evidenceCandidateCount(issue)" :key="candidate" type="button" @click.stop="emit('evidenceSelect', { issueId: issue.id, index: candidate - 1 })">{{ t('contractReview.evidenceCandidate', { number: candidate }) }}</button>
            </div>
...
function evidenceCandidateCount(issue: ReviewIssue) {
  return props.evidenceCandidateCounts?.[issue.id] || 0
}
...
.evidence-status{font-size:10px;color:var(--legal-text-secondary)}.evidence-status--located{color:var(--legal-ai-strong)}.evidence-status--legacy_exact,.evidence-status--pending{color:var(--legal-warning-strong)}.evidence-status--multiple_matches,.evidence-status--not_found,.evidence-status--version_mismatch,.evidence-status--unsupported,.evidence-status--error{color:var(--legal-risk-strong)}
.evidence-candidates{display:flex;align-items:center;flex-wrap:wrap;gap:5px;margin-top:8px;color:var(--legal-risk-strong);font-size:10px}.evidence-candidates button{padding:3px 6px;border:1px solid var(--legal-border);border-radius:3px;color:var(--legal-text-primary);background:var(--legal-bg-surface);font-size:10px;cursor:pointer;&:hover{border-color:var(--legal-brand);color:var(--legal-brand)}&:focus-visible{outline:2px solid var(--legal-focus-ring);outline-offset:1px;}}
