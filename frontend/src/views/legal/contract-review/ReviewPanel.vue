...
import type { ContractReview, RepresentedParty, ReviewClause, ReviewIssue, ReviewPlaybook, RiskLevel } from '@/api/contract-review'
import type { ModelConfig } from '@/api/model'

const props = defineProps<{ review: ContractReview; playbooks: ReviewPlaybook[]; models: ModelConfig[]; modelsLoading?: boolean; selectedIssueId?: string; busy?: boolean; reconfigure?: boolean }>()
const emit = defineEmits<{ start: []; retry: []; reconfigure: []; cancelReconfigure: []; configure: []; issueSelect: [issue: ReviewIssue]; configChange: [data: { playbook_id?: string; represented_party?: RepresentedParty; model_id?: string }] }>()
const { t } = useI18n()
