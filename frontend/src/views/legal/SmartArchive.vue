      ...
      <template v-if="tab === 'documents'">
        <section class="document-panel">
          <div class="document-panel__heading">
            ...
            <div v-if="selectedCount" class="bulk-toolbar"><span>{{ t('smartArchive.selectedCount', { count: selectedCount }) }}</span><template v-if="!showArchived"><button v-if="canContribute" data-testid="archive-bulk-archive" class="secondary-button bulk-document-action" type="button" :disabled="bulkWorking" @click="runBulkAction('archive')"><t-icon name="archive" /><span>{{ t('smartArchive.bulkArchive') }}</span></button><button v-if="canAdmin" data-testid="archive-bulk-purge" class="secondary-button secondary-button--danger bulk-document-action" type="button" :disabled="bulkWorking" @click="runBulkAction('purge')"><t-icon name="delete" /><span>{{ t('smartArchive.bulkDelete') }}</span></button></template><template v-else-if="canAdmin"><button data-testid="archive-bulk-restore" class="secondary-button bulk-document-action" type="button" :disabled="bulkWorking" @click="runBulkAction('restore')"><t-icon name="rollback" /><span>{{ t('smartArchive.bulkRestore') }}</span></button><button data-testid="archive-bulk-purge" class="secondary-button secondary-button--danger bulk-document-action" type="button" :disabled="bulkWorking" @click="runBulkAction('purge')"><t-icon name="delete" /><span>{{ t('smartArchive.bulkDelete') }}</span></button></template></div>
          </div>
...
const extractionStatuses: ArchiveExtractionStatus[] = ['uploading', 'parsing', 'extracting', 'linking', 'needs_review', 'completed', 'failed', 'canceled']
...
const hasProcessingDocuments = computed(() => store.documents.some((document) => ['uploading', 'parsing', 'extracting', 'linking'].includes(document.extraction_status) || document.mirror_status === 'pending' || document.mirror_status === 'processing') || Boolean(selected.value && ['uploading', 'parsing', 'extracting', 'linking'].includes(selected.value.extraction_status)))
...
async function refreshSelectedDocument() {
  ...
  if (!selectedId || selectedRefreshInFlight) return
  selectedRefreshInFlight = true
  try {
    const document = await fetchSelectedDocument(selectedId)
    if (document && selected.value?.id === selectedId) selected.value = document
  } finally {
    selectedRefreshInFlight = false
  }
}
async function fetchSelectedDocument(id: string): Promise<ArchiveDocument | null> {
  const generation = ++selectedRefreshGeneration
  try {
    const document = await store.loadDocument(id)
    return generation === selectedRefreshGeneration ? document : null
  } catch { /* Keep the last known document while a refresh is transiently unavailable. */ }
  return null
}
...
async function selectTab(value: string) { selectedIds.value = []; reminderSelectedIds.value = []; candidateSelectedIds.value = []; selectedRefreshGeneration++; selected.value = null; tab.value = value; if (value === 'documents') await refreshDocumentSearch(); if (value === 'queue') await store.loadDocuments('', false); if (value === 'reminders') await loadReminders() }
...
async function openDocument(document: ArchiveDocument) { selected.value = document; const loaded = await fetchSelectedDocument(document.id); if (loaded && selected.value?.id === document.id) selected.value = loaded }
function closeSelectedDocument() { selectedRefreshGeneration++; selected.value = null }
async function archiveDocument(document: ArchiveDocument | null) { if (!document || !window.confirm(t('smartArchive.archiveConfirm', { name: document.title }))) return; try { await store.archive(document.id); selectedIds.value = selectedIds.value.filter((id) => id !== document.id); selectedRefreshGeneration++; selected.value = null; await reloadDocumentContext() } catch (e: any) { MessagePlugin.error(e?.message || t('smartArchive.archiveFailed')) } }
...
async function cancelExtraction(document: ArchiveDocument) { if (!window.confirm(t('smartArchive.cancelExtractionConfirm', { name: document.title }))) return; selectedRefreshGeneration++; try { await store.cancelExtraction(document.id); MessagePlugin.success(t('smartArchive.cancelExtractionSuccess')); await reloadDocumentContext(); const refreshed = await fetchSelectedDocument(document.id); if (refreshed && selected.value?.id === document.id) selected.value = refreshed } catch (e: any) { MessagePlugin.error(e?.message || t('smartArchive.cancelExtractionFailed')) } }
async function retryExtraction(document: ArchiveDocument) { selectedRefreshGeneration++; try { await store.retryExtraction(document.id); MessagePlugin.success(t('smartArchive.importStarted')); await reloadDocumentContext(); const refreshed = await fetchSelectedDocument(document.id); if (refreshed && selected.value?.id === document.id) selected.value = refreshed } catch (e: any) { MessagePlugin.error(e?.message || t('smartArchive.importFailed')) } }
async function retryMirror(document: ArchiveDocument) { selectedRefreshGeneration++; try { await store.retryMirror(document.id); MessagePlugin.success(t('smartArchive.mirrorRetryStarted')); await reloadDocumentContext(); const refreshed = await fetchSelectedDocument(document.id); if (refreshed && selected.value?.id === document.id) selected.value = refreshed } catch (e: any) { MessagePlugin.error(e?.message || t('smartArchive.mirrorRetryFailed')) } }
...
async function openCandidateSource(candidate: ArchiveReminderCandidate) { const document = await fetchSelectedDocument(candidate.document_id); if (document) selected.value = document }
...
onMounted(() => { window.addEventListener('resize', onResize); document.addEventListener('click', closeStatusFilterOnOutsideClick, true); void refreshDocumentSearch(); void Promise.allSettled([store.loadSettings(), loadMembers()]); void loadReminders(); syncStatusPolling(hasProcessingDocuments.value) }); onBeforeUnmount(() => { window.removeEventListener('resize', onResize); document.removeEventListener('click', closeStatusFilterOnOutsideClick, true); stopStatusPolling(); store.disconnect() })
</script>
...
.archive-status-dot--canceled { background: var(--legal-text-secondary); }
...
.archive-document-status--canceled { color: var(--legal-text-secondary); }
.archive-document-status--canceled i { background: var(--legal-text-secondary); box-shadow: 0 0 0 3px var(--legal-border); }
