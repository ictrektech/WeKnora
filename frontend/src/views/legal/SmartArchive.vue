...
import { archiveDocumentDisplayStatus, archiveDocumentStatusTone, buildArchiveSearchFilters, hasMoreArchiveDocuments } from './smartArchiveDocuments'
...
const hasProcessingDocuments = computed(() => store.documents.some((document) => ['uploading', 'parsing', 'extracting', 'linking'].includes(document.extraction_status) || document.mirror_status === 'pending' || document.mirror_status === 'processing'))
...
async function cancelExtraction(document: ArchiveDocument) { if (!window.confirm(t('smartArchive.cancelExtractionConfirm', { name: document.title }))) return; try { await store.cancelExtraction(document.id); MessagePlugin.success(t('smartArchive.cancelExtractionSuccess')); await reloadDocumentContext(); const refreshed = await store.loadDocument(document.id); if (refreshed) selected.value = refreshed } catch (e: any) { MessagePlugin.error(e?.message || t('smartArchive.cancelExtractionFailed')) } }
...
async function retryMirror(document: ArchiveDocument) { try { await store.retryMirror(document.id); MessagePlugin.success(t('smartArchive.mirrorRetryStarted')); await reloadDocumentContext(); const refreshed = await store.loadDocument(document.id); if (refreshed) selected.value = refreshed } catch (e: any) { MessagePlugin.error(e?.message || t('smartArchive.mirrorRetryFailed')) } }
