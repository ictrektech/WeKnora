...
const MARKDOWN_SAMPLE = `# HybRAG 知识框架

HybRAG 是一个基于 LLM 的开源企业知识框架，集 RAG 问答、ReAct 智能体、Wiki 知识图谱于一体。本文介绍其设计动机、架构与典型用法。

## 设计动机

企业内部知识散落在 Confluence、飞书、Notion、Git 仓库等多种系统，传统全文检索难以理解语义，单一 LLM 又缺少可信赖的上下文来源。HybRAG 的目标是：
...
\`\`\`bash
git clone https://github.com/Tencent/HybRAG && cd HybRAG
...
const FAQ_SAMPLE = `# HybRAG 部署与使用 FAQ
